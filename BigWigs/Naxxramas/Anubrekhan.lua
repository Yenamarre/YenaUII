...
L:RegisterTranslations("enUS", function() return {
	...
	alert_cmd = "kittig",
	alert_name = "phase kitting",
	alert_desc = "Masque l'annonce du début de la phase kitting",
...
L:RegisterTranslations("frFR", function() return {
	...
	starttrigger3 = "Nul part pour s'enfuir.",
...
BigWigsAnubrekhan.toggleoptions = {"locust", "alert", "bosskill"}
...
function BigWigsAnubrekhan:BigWigs_RecvSync( sync )
	if sync == "AnubLocustInc" then
		...
		if self.db.profile.locust then
			...
			if not self.db.profile.alert then
				self:TriggerEvent("BigWigs_Message", L["castwarn"], "Personal", true, "Long")
			end
		end
