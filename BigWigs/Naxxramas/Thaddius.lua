...
L:RegisterTranslations("enUS", function() return {
	...
	addsdownwarn = "Thaddius incoming in 15 sec!",
	thaddiusincoming = "Thaddius incoming !",
...
L:RegisterTranslations("frFR", function() return {
	...
	addsdownwarn = "Thaddius arrive dans 15sec !",
	thaddiusincoming = "Thaddius arrive !",

	pstrigger = "Maintenant TOI sentir douleur !",
	trigger1 = "Thaddius commence à lancer Changement de polarité.",
	trigger2 = "Thaddius lance Changement de polarité.",
...
function BigWigsThaddius:CHAT_MSG_MONSTER_YELL( msg )
	if msg == L["starttrigger"] or msg == L["starttrigger1"] then
		if self.db.profile.phase and not self.stage1warn then
			self:TriggerEvent("BigWigs_Message", L["startwarn"], "Important")
		end
		self.stage1warn = true
		self:Throw()
		self:ScheduleRepeatingEvent( "bwthaddiusthrow", self.Throw, 20, self )
	elseif msg == L["adddeath"] or msg == L["adddeath2"] then
		...
		if self.addsdead == 2 then
			...
			self:TriggerEvent("BigWigs_StartBar", self, L["thaddiusincoming"], 15, "Interface\\Icons\\Spell_Nature_Lightning")
			self:CancelScheduledEvent("bwthaddiusthrow")
			self:CancelScheduledEvent("bwthaddiusthrowwarn")
			self:TriggerEvent("BigWigs_Message", "|cffFF0000--Rouge--|r |cff00FF00<==Boss==>|r |cff0026FF++Bleu++|r", "Positive", true, "Alarm")
		end
	elseif string.find(msg, L["starttrigger2"]) or string.find(msg, L["starttrigger3"]) or string.find(msg, L["starttrigger4"]) then
		if self.db.profile.phase then
			...
			self:TriggerEvent("BigWigs_StartBar", self, L["bar1text"], 15, "Interface\\Icons\\Spell_Nature_Lightning")
		end
...
function BigWigsThaddius:PolarityCast( msg )
	...
	elseif self.db.profile.polarity and string.find(msg, L["trigger2"]) then
...
function BigWigsThaddius:CheckPolarity()
...
function BigWigsThaddius:BigWigs_RecvSync( sync )
	-- if sync == "ThaddiusPolarity" and self.db.profile.polarity then
		-- self:ScheduleEvent(function() self:CheckPolarity() end, 3.1)
		-- self:ScheduleEvent("BigWigs_Message", 27, L["pswarn3"], "Important")
		-- self:TriggerEvent("BigWigs_StartBar", self, L["bar1text"], 30, "Interface\\Icons\\Spell_Nature_Lightning")
	-- else
