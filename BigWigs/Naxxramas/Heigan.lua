...
L:RegisterTranslations("frFR", function() return {
	...
	-- [[ Warnings ]]--
	engage_message = "Heigan l'Impur engagé! 90 sec avant la Téléportation",
	...
	on_platform_message = "Phase Danse!",

	to_floor_30sec_message = "Reprise dans 30 sec",
	to_floor_10sec_message = "Reprise dans 10 sec!",
	on_floor_message = "Phase Tanking!",

	-- [[ Bars ]]--
	teleport_bar = "Téléportation",
	back_bar = "Phase Danse",
} end )
...
function BigWigsHeigan:CHAT_MSG_MONSTER_YELL( msg )
	if string.find(msg, L["starttrigger"]) or string.find(msg, L["starttrigger2"]) or string.find(msg, L["starttrigger3"]) then
		...
		if self.db.profile.teleport then
			...
			self:ScheduleEvent("bwheiganwarn2", "BigWigs_Message", self.toPlatformTime-30, L["teleport_30sec_message"], "Urgent", false, "Info")
			self:ScheduleEvent("bwheiganwarn3", "BigWigs_Message", self.toPlatformTime-10, L["teleport_10sec_message"], "Important", false, "Alert")
			self:ScheduleEvent( self.PhaseDanse, self.toPlatformTime, self)
		end
	elseif string.find(msg, L["die_trigger"]) then
...
function BigWigsHeigan:PhaseTanking()
	if self.db.profile.teleport then
		self:TriggerEvent("BigWigs_Message", L["on_floor_message"], "Attention", false, "Alarm")
		self:TriggerEvent("BigWigs_StartBar", self, L["teleport_bar"], self.toPlatformTime, "Interface\\Icons\\Spell_Arcane_Blink")
		self:ScheduleEvent("bwheiganwarn1", "BigWigs_Message", self.toPlatformTime-60, L["teleport_1min_message"], "Attention")
		self:ScheduleEvent("bwheiganwarn2", "BigWigs_Message", self.toPlatformTime-30, L["teleport_30sec_message"], "Urgent", false, "Info")
		self:ScheduleEvent("bwheiganwarn3", "BigWigs_Message", self.toPlatformTime-10, L["teleport_10sec_message"], "Important", false, "Alert")
...
function BigWigsHeigan:PhaseDanse()
	if self.db.profile.teleport then
		self:TriggerEvent("BigWigs_Message", L["on_platform_message"], "Attention", false, "Alarm")
		self:ScheduleEvent("bwheiganwarn2","BigWigs_Message", self.toRoomTime-30, L["to_floor_30sec_message"], "Urgent", false, "Info")
		self:ScheduleEvent("bwheiganwarn3","BigWigs_Message", self.toRoomTime-10, L["to_floor_10sec_message"], "Important", false, "Alert")
		self:TriggerEvent("BigWigs_StartBar", self, L["back_bar"], self.toRoomTime, "Interface\\Icons\\Ability_Creature_Disease_02")
		self:ScheduleEvent( self.PhaseTanking, self.toRoomTime, self)
	end
end