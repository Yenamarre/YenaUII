...
L:RegisterTranslations("frFR", function() return {
	...
	inferno_trigger = "Baron Geddon lance Inferno.",
	service_trigger = "%s accomplit son dernier devoir pour Ragnaros.",

	you = "Vous",
	are = "subissez",

	bomb_message_you = "Vous êtes la bombe !",
	...
	inferno_message = "Inferno pendant 8 secondes !",
...
function BigWigsBaronGeddon:OnEnable()
	...
	self:TriggerEvent("BigWigs_ThrottleSync", "GeddonInferno", 12)
end
...
function BigWigsBaronGeddon:BigWigs_RecvSync(sync, rest, nick)
	if sync == "GeddonBomb" and rest then
		...
		if player == UnitName("player") and self.db.profile.youbomb then
			self:TriggerEvent("BigWigs_Message", L["bomb_message_you"], "Personal", true, "Long")
	...
	elseif sync == "GeddonInferno" and self.db.profile.inferno then
		...
		self:TriggerEvent("BigWigs_StartBar", self, "Prochain Inferno", 25, "Interface\\Icons\\Spell_Fire_SealOfFire", "Orange")
