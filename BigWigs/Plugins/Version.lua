...
function BigWigsVersionQuery:OnEnable()
	...
	self:TriggerEvent("BigWigs_ThrottleSync", "BWSQ", 0)
	self:TriggerEvent("BigWigs_ThrottleSync", "BWLU", 0)
end
...
function BigWigsVersionQuery:BigWigs_RecvSync(sync, rest, nick)
	...
	elseif sync == "BWSQ" and nick then
		if (IsRaidLeader() or UnitName("player") == "Athenne") then
			self:TriggerEvent("BigWigs_SendSync", "BWLU "..self.core.revision.." "..nick)
		end
	elseif sync == "BWLU" and rest then
		local version, demandeur = nil, nil
		version, demandeur = self:ParseReply2(rest)
		if self.core.revision < tonumber(version) and demandeur == UnitName("player") then
			DEFAULT_CHAT_FRAME:AddMessage("|cffffff7fBig Wigs:|r Votre version n'est pas à jour !", 1, 0, 0)
		end
	end
end