...
L:RegisterTranslations("frFR", function() return {
	...
	["Message frame scale"] = "Taille du texte des messages",
...
BigWigsMessages.consoleOptions = {
	...
	args   = {
		...
		["Chat"] = {
			type = "toggle",
			name = "Chat",
			desc = "Affiche également les messages dans la fenêtre de discussion",
			get = function() return BigWigsMessages.db.profile.chat end,
			set = function(v) BigWigsMessages.db.profile.chat = v end,
		}
	},
}
...
function BigWigsMessages:CreateMsgFrame()
	...
	self.msgframe:SetTimeVisible(4)
	self.msgframe:SetFadeDuration(2)
	self.msgframe:SetFrameStrata("DIALOG")
	self.msgframe:SetToplevel(true)
	self.msgframe:SetFontObject(BigWigsMsgFont)
	self.msgframe:Show()
end
...
function BigWigsMessages:BigWigs_Message(text, color, noraidsay, sound, broadcastonly)
	...
	elseif CombatText_AddMessage and self.db.profile.display == L["Floating Combat Text"] then -- Blizzards FCT
		...
		self.msgframe:AddMessage(text, r, g, b, 1, 3)
	end
	
	if self.db.profile.chat then
		DEFAULT_CHAT_FRAME:AddMessage("|cffffff7fBig Wigs:|r "..text, r, g, b)
	end
...
function BigWigsMessages:SetupFrames()
	...
	f = "Interface\\AddOns\\BigWigs\\Fonts\\Porky.ttf"
	...
	self.frames.cheader:SetFont(f, 18, "OUTLINE")
