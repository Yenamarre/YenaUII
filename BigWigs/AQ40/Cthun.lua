...
L:RegisterTranslations("frFR", function() return {
	...
	tentacle1	= "Arrivée des Tentacules oculaire!",
	tentacle2	= "Tentacules oculaire dans 5 sec",

	norape1		= "Arrivée des Tentacules oculaire!",
	norape2		= "Tentacules oculaire dans 5 sec",

	weakened		= "C'Thun est affaibli pour 45 sec",
	invulnerable2	= "Affaiblissement termine dans 5 seconds",
	invulnerable1	= "Affaiblissement terminé - C'Thun est invulnerable",

	giant3		= "Oculaire géant - 10 sec",
	giant2		= "Oculaire géant - 5 sec",
	giant1		= "Oculaire géant!",

	startwarn	= "C'Thun engagé! - 45 sec avant le Sombre regard et les oculaires",

	glare2		= "Sombre regard - 5 sec",
	glare1		= "Sombre regard!",

	barTentacle	= "Tentacules oculaire!",
	barNoRape	= "Tentacules oculaire!",
	barWeakened	= "C'Thun est affaibli!",
	barGlare	= "Prochain Sombre Regard",
	barGiant	= "Oculaire Géant!",
	barGreenBeam	= "Phase Rayon vert",

	eyebeam		= "Rayon de l'Oeil",
	glarewarning	= "SOMBRE REGARD SUR VOUS!",
	groupwarning	= "Sombre regard sur le groupe %s (%s)",
	positions1	= "Sombre regard terminé - Espacez-vous!",
	positions2	= "Sombre regard termine dans 5 sec",
	phase2starting	= "Phase 2!",
} end )
...
function BigWigsCThun:OnEnable()
	...
	self:RegisterEvent("CHAT_MSG_SPELL_PERIODIC_SELF_DAMAGE") -- Acide digestif
	...
	self:TriggerEvent("BigWigs_ThrottleSync", "CThunStomach", 5)
	
	if CThunWarnerFrame and not CThunWarnerStatus_CheckRunning then
		CThunWarnerFrame:Show();
	end
end

function BigWigsCThun:OnDisable()
	if CThunWarnerFrame then
		CThunWarner_Off();
...
function BigWigsCThun:CHAT_MSG_SPELL_CREATURE_VS_CREATURE_DAMAGE(arg1)
	if not cthunstarted and arg1 and string.find(arg1, L["eyebeam"]) then self:TriggerEvent("BigWigs_SendSync", "CThunStart") end
end
...
function BigWigsCThun:CHAT_MSG_SPELL_PERIODIC_SELF_DAMAGE(msg)
	if string.find(msg, "Vous subissez les effets de Acide digestif.") then
		self:TriggerEvent("BigWigs_SendSync", "CThunStomach "..Player)
	end
end
...
function BigWigsCThun:BigWigs_RecvSync(sync, rest)
	...
	elseif sync == "CThunWeakened" then
		self:CThunWeakened()
	elseif sync == "CThunStomach" and rest then
		self:TriggerEvent("BigWigs_Message", string.format("%s est dans le ventre de C'Thun!", rest), "Positive")
	end
end
...
function BigWigsCThun:CThunStart()
	if not cthunstarted then
		...
		self:ScheduleEvent("bwcthungroupwarningstart", self.GroupWarning, timeP1GlareStart - 2.8, self )
...
function BigWigsCThun:GroupWarning()
	if target then
		...
		for i = 1 , GetNumRaidMembers() do
			local name2, _, subgroup, _, _, _, _, _ = GetRaidRosterInfo(i)
			if (name2 == UnitName("PLAYER")) then
				if subgroup == group then
					self:TriggerEvent("BigWigs_Message", "Sombre Regard sur votre groupe !", "Personal", true, "Long")
				end
			end
		end
