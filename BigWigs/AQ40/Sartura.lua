...
L:RegisterTranslations("frFR", function() return {
	...
	starttrigger = "Vous serez jugé pour avoir profané ces lieux sacrés.",
...
function BigWigsSartura:BigWigs_RecvSync(sync)
	if sync == "SarturaWhirlwind" and self.db.profile.whirlwind then
		...
		self:TriggerEvent("BigWigs_StartBar", self, "Prochain tourbillon", 26, "Interface\\Icons\\Ability_Whirlwind")
	end
end
...
function BigWigsSartura:CHAT_MSG_MONSTER_YELL(msg)
	if self.db.profile.enrage and string.find(msg, L["starttrigger"]) then
		...
		self:TriggerEvent("BigWigs_StartBar", self, "Prochain tourbillon", 15, "Interface\\Icons\\Ability_Whirlwind")
