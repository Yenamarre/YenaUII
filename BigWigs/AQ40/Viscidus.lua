...
L:RegisterTranslations("frFR", function() return {
	...
	trigger1 	= "commence à ralentir.",
	trigger2 	= "commence à geler.",
	trigger3 	= "est un bloc congelé.",
	trigger4 	= "commence à se fissurer.",
	trigger5 	= "semble prêt à s'effondrer.",
	trigger6	= "les effets de Salve d'éclairs de poison",
	trigger7 	= "^([^%s]+) ([^%s]+) les effets de Toxine%.$",

	you 		= "Vous",
	are 		= "subissez",

	warn1 		= "Première phase de Gel!",
	warn2 		= "Deuxième phase de gel!",
	warn3 		= "Viscidus est gelé!",
	warn4 		= "Début de phase fissure!",
	warn5 		= "Viscidus va exploser!",
	warn6		= "Poison Bolt Volley!",
	warn7		= "Poison Bolt Volley in ~3 sec!",
	warn8		= " est dans un nuage de Toxines!",
	warn9		= "Vous êtes dans un nuage de Toxines!",

	bar1text	= "Salve d'éclairs de poison",
} end )
	...
	elseif string.find(arg1, L["trigger7"]) then
		...
		if (pl and ty) then
			if self.db.profile.toxinyou and pl == L["you"] and ty == L["are"] then
				self:TriggerEvent("BigWigs_Message", L["warn9"], "Personal", true, "Long")
...
function BigWigsViscidus:CHAT_MSG_MONSTER_EMOTE(arg1)
	...
	if string.find(arg1, L["trigger1"]) then
	...
	elseif string.find(arg1, L["trigger3"]) then
		self:TriggerEvent("BigWigs_Message", L["warn3"], "Important")
	elseif string.find(arg1, L["trigger4"]) then
		self:TriggerEvent("BigWigs_Message", L["warn4"], "Urgent")
	elseif string.find(arg1, L["trigger5"]) then
