...
local warnpairs = nil
local addcounter = nil
local type1 = nil
local type1counter = nil
local type2 = nil
local type2counter = nil
local chromacounter = nil
...
L:RegisterTranslations("frFR", function() return {
	...
	triggershamans	= "Chaman",
	triggerdruid	= "Druide",
	triggerwarlock	= "Démoniste",
	triggerpriest	= "Prêtre",
	triggerhunter	= "Chasseurs",
	triggerwarrior	= "Guerrier",
	triggerrogue	= "Voleur",
	triggerpaladin	= "Paladin",
	triggermage		= "Mages",
...
function BigWigsNefarian:CHAT_MSG_MONSTER_YELL(msg)
	-- if string.find(msg, L["landing_soon_trigger"]) then
		-- BigWigs:Print("Drakônide Chromatique: "..chromacounter)
		-- BigWigs:Print("Drakônide "..type1..": "..type1counter)
		-- BigWigs:Print("Drakônide "..type2..": "..type2counter)
	-- end
	for i,v in pairs(warnpairs) do
		if string.find(msg, i) then
			if v[2] then
				if self.db.profile.classcall then
					self:TriggerEvent("BigWigs_Message", v[1], "Personal", true, "Long")
...
function BigWigsNefarian:CHAT_MSG_COMBAT_HOSTILE_DEATH(msg)
	local _,_,unit = string.find(msg, "Drakônide (%a+) meurt.")
	if unit ~= "chromatique" and not type1 and string.find(msg, "Drakônide") then
		type1 = unit
		type1counter = type1counter + 1
	elseif unit ~= "chromatique" and unit ~= type1 and not type2 and string.find(msg, "Drakônide") then
		type2 = unit
		type2counter = type2counter + 1
	elseif unit == type1 and string.find(msg, "Drakônide") then
		type1counter = type1counter + 1
		if type1counter >= 17 and type1counter < 20 then
			self:TriggerEvent("BigWigs_Message", string.format("Encore %d Drakônide %s", 20-type1counter, type1), "Positive")
		end
	elseif unit == type2 and string.find(msg, "Drakônide") then
		type2counter = type2counter + 1
		if type2counter >= 17 and type2counter < 20 then
			self:TriggerEvent("BigWigs_Message", string.format("Encore %d Drakônide %s", 20-type2counter, type2), "Positive")
		end
	elseif unit == "chromatique" and string.find(msg, "Drakônide") then
		chromacounter = chromacounter + 1
		if chromacounter >= 9 and chromacounter < 12 then
			self:TriggerEvent("BigWigs_Message", string.format("Encore %d Drakônide chromatique", 12-chromacounter), "Positive")
		end
	end
	
	if unit ~= "chromatique" and string.find(msg, "Drakônide") then --"Drakônide Chromatique/rouge/noir/... meurt.", "Drakônide (%a+) meurt.")
		addcounter = addcounter + 1
		self:TriggerEvent("BigWigs_Message", string.format("%d Drakônide tués !", addcounter), "Positive")
	end
end
