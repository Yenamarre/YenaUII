...
L:RegisterTranslations("enUS", function() return {
	...
	winds_trigger = "^(.+) (.+) les effets de Linceul de vents%.",
	winds_message = "Linceul sur %s",
...
L:RegisterTranslations("frFR", function() return {
	...
	winds_trigger = "^([^%s]+) ([^%s]+) les effets de Linceul de vents%.$",
	winds_message = "Linceul sur %s",
...
function BigWigsOssirian:OnEnable()
	...
	self:TriggerEvent("BigWigs_ThrottleSync", "OssirianWinds", 10)
	self:TriggerEvent("BigWigs_ThrottleSync", "OssirianStomp", 10)
	self:TriggerEvent("BigWigs_ThrottleSync", "WindsTarget", 1)
end

function BigWigsOssirian:Event(msg)
	local _, _, EPlayer, EType = string.find(msg, L["winds_trigger"])
	if EPlayer then
		if EPlayer == "Vous" then
			EPlayer = UnitName("player")
		end
		self:TriggerEvent("BigWigs_SendSync", "WindsTarget "..EPlayer)
	end

	if string.find(msg, "Linceul de vents") then
		self:TriggerEvent("BigWigs_SendSync", "OssirianWinds")
	elseif string.find(msg, "Choc martial") then
		self:TriggerEvent("BigWigs_SendSync", "OssirianStomp")
	end
end
...
function BigWigsOssirian:BigWigs_RecvSync(sync, rest)
	if sync == "WindsTarget" and rest then
		local player = rest
		self:TriggerEvent("BigWigs_StartBar", self, string.format(L["winds_message"], player), 10, "Interface\\Icons\\Spell_Nature_Cyclone")
	elseif sync == "OssirianWinds" then
		self:TriggerEvent("BigWigs_StartBar", self, "Linceul de vents", 20, "Interface\\Icons\\Spell_Nature_Cyclone", "Orange")
	elseif sync == "OssirianStomp" then
		self:TriggerEvent("BigWigs_StartBar", self, "Choc martial", 30, "Interface\\Icons\\Ability_WarStomp", "Red")
	end
	
	if sync ~= "OssirianWeakness" or not rest or not L:HasTranslation(rest) then return end

	if self.db.profile.debuff then
		self:TriggerEvent("BigWigs_Message", string.format(L["debuffwarn"], L[rest]), "Important")
	end
