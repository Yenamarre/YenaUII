...
L:RegisterTranslations("frFR", function() return {
	...
	warn9 = "Vague 1/7", -- trigger for starting the event by pulling the first wave instead of talking to andorov
} end )
...
function BigWigsGeneralRajaxx:CHAT_MSG_MONSTER_YELL( msg )
	if self.db.profile.wave and msg then
		if (string.find(msg, L["trigger1"])) then
			self:TriggerEvent("BigWigs_Message", L["warn1"], "Urgent")
		...
		elseif (string.find(msg, L["trigger3"])) then
			self:TriggerEvent("BigWigs_Message", L["warn3"], "Urgent")
		...
		elseif (string.find(msg, L["trigger5"])) then
			self:TriggerEvent("BigWigs_Message", L["warn5"], "Urgent")
		elseif (string.find(msg, L["trigger6"])) then
			self:TriggerEvent("BigWigs_Message", L["warn6"], "Urgent")
		elseif (string.find(msg, L["trigger7"])) then
			self:TriggerEvent("BigWigs_Message", L["warn7"], "Urgent")
		elseif (string.find(msg, L["trigger8"])) then
			self:TriggerEvent("BigWigs_Message", L["warn8"], "Urgent")
		elseif (string.find(msg, L["trigger9"])) then
			self:TriggerEvent("BigWigs_Message", L["warn9"], "Urgent")
		end
	end
end
