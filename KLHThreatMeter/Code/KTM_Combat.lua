...
me.specialattack = function(abilityid, target, damage, iscrit, spellschool)
	...
	elseif abilityid == "heroicstrike" then
		...
		reduc =  (armor / ((85 * level) + armor + 400)) * 100
		
		local dd_HS = mod.my.ability("heroicstrike", "nextattack") 
		
		if iscrit then
		dd_HS = dd_HS *2
		end
		local dd_HS_reduc = dd_HS * (100-reduc)/100
		local whitedamage = damage - dd_HS_reduc
	...
	--	local whitedamage = preimpaledamage * (myaveragedamage / (addeddamage + myaveragedamage))
		...
		me.event.damage = damage
		me.event.threat = threatmodifier * (dd_HS_reduc + mod.my.ability("heroicstrike", "threat"))
		me.event.rage = mod.my.ability("heroicstrike", "rage") + whitedamage / (UnitLevel("player") / 2)
	
	if mes_sartas_HS then
			mod.out.printtrace(string.format("Armure de la cible : "..armor.."  pourcentage de reduc d'armure :"..math.floor(reduc).."  HS damage:"..damage.."  dd_HS:"..dd_HS.."   dd_HS_reduit:"..math.floor(dd_HS_reduc).."   white damege : "..math.floor(whitedamage).."  ---- Threat : "..math.floor(me.event.threat).."---- By sartas Calculator----"))
	...
	elseif abilityid == "maul" then
		...
		-- mod.out.printtrace(string.format("level:"..level.."target level"..targetlevel))
		--[[ ancien calculator avec une approximation de la partie blanche des degats du maul
		...
		local myaveragedamage = me.averagemainhanddamage()
		local lastwhitedamage = presavagefurydamage * (myaveragedamage / (addeddamage + myaveragedamage))
		]]
		-- Now make a separate method call for the autoattack component
		--me.specialattack("whitedamage", target, whitedamage, nil, nil)
		
		-- Nouveau calcul en prenant en compte la reduction de degats de l'armure de la cible
		local ddmaul = mod.my.ability("maul", "nextattack") * (1 + mod.my.mods.druid.savagefury)
		if iscrit then
		ddmaul = ddmaul *2
		end
		local ddmaulreduc = ddmaul * (100-reduc)/100
		local whitedamage = damage - ddmaulreduc
		
		me.specialattack("whitedamage", target, whitedamage, nil, nil)
		
		-- The above method will have overwritten some parts of me.event, so change them back
		me.event.damage = damage
		me.event.threat = threatmodifier * mod.my.ability("maul", "multiplier") * ddmaulreduc 
		me.event.rage = mod.my.ability("maul", "rage") + whitedamage / (UnitLevel("player") / 2)
		
		if mes_sartas_maul then
			mod.out.printtrace(string.format("Armure de la cible : "..armor.."  pourcentage de reduc d'armure :"..math.floor(reduc).."  Maul damage:"..damage.."  ddmaul:"..ddmaul.."   ddmaulreduit:"..math.floor(ddmaulreduc).."   white damege : "..math.floor(whitedamage).."  ---- Threat : "..math.floor(me.event.threat).."---- By sartas Calculator----"))
	...
	elseif mod.my.class == "rogue" then
		...
		if mod.my.mods.rogue.bonescythe == true then
			if (spellid == "sinisterstrike") or (spellid == "backstab") or (spellid == "eviscerate") or (spellid == "hemorrhage") then
				...
				if mes_sartas_rogue then
			mod.out.printtrace(string.format("6 part 3 reduction damage = "..me.event.damage.."  --  Menace :"..me.event.threat.."---- By sartas tester----"))
...
me.taunt = function(target)
	...
	--sartas à commenter
	if mes_sartas_taunt then
	mod.out.printtrace(string.format("Taunting Detected "..target.."!!!!!!!!! by sartas", target))
	end
...
-- Ajout luciole
me.luciole = function(target)
	local threatmodifier = mod.my.globalthreat.value
	--sartas à commenter
	if mes_sartas then
	mod.out.printtrace(string.format("luciole Detected "..target.."!!!!!!!!! by sartas", target))
	end
	
		
			me.event.damage = 0
			me.event.threat = threatmodifier * mod.my.ability("luciole", "threat")
			me.event.rage = mod.my.ability("luciole", "rage")
			me.event.hits = 1
			me.event.name = mod.string.get("spell", "luciole")

			me.addattacktodata(mod.string.get("spell", "luciole"), me.event)
			me.addattacktodata(mod.string.get("threatsource", "total"), me.event)
	if mes_sartas_taunt then
		mod.out.printtrace(string.format("Your luciole on : "..target.." gaining "..me.event.threat.. "threat."))
	end		
...
me.Demoralizing = function(target)
	local threatmodifier = mod.my.globalthreat.value
	--sartas à commenter
	if mes_sartas then
	mod.out.printtrace(string.format("cri demorailsant Detected "..target.."!!!!!!!!! by sartas", target))
	end
	
		
			me.event.damage = 0
			me.event.threat = threatmodifier * mod.my.ability("Demoralizing", "threat")
			me.event.rage = mod.my.ability("Demoralizing", "rage")
			me.event.hits = 1
			me.event.name = mod.string.get("spell", "Demoralizing")

			me.addattacktodata(mod.string.get("spell", "Demoralizing"), me.event)
			me.addattacktodata(mod.string.get("threatsource", "total"), me.event)
	if mes_sartas then
		mod.out.printtrace(string.format("Your Demoralizing on : "..target.." gaining "..me.event.threat.. "threat. -- Threat modifier :"..threatmodifier..".--"))
	end		
			
end

me.miss = function(target,spellid,spellname)
	
	--- sartas retract sunder miss
	...
	if spellname == "Lucioles" or spellname == "Lucioles (farouche)" or spellname == "Faerie Fire" or spellname == "Faerie Fire (Feral)" then
		...
		local threatmodifier = mod.my.globalthreat.value
		me.event.threat = - mod.my.ability("luciole", "threat") * threatmodifier 
		...
		spellid = spellid or spellname
		if mes_sartas_taunt then
				mod.out.printtrace(string.format("--1--We retract "..me.event.threat.." Resist on your"..spellid))
		end
...
me.powergain = function(amount, powertype, spellid)
	...
	elseif powertype == mod.string.get("power", "energy") then
		me.event.threat = amount * mod.data.threatconstants.energygain * mod.my.globalthreat.value
		
	elseif powertype == mod.string.get("power", "mana") then
		me.event.threat = amount * mod.data.threatconstants.managain  * mod.my.globalthreat.value
